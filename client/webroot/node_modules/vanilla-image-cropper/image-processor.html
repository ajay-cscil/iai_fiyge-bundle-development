...
<body >
<div class="tab">
	  <button class="tablinks" onclick="switchEditor(event, 'tabcontent-draw-image')"  id="defaultOpen">Draw Sign</button>
	  <button class="tablinks" onclick="switchEditor(event, 'tabcontent-upload-image')">Upload Sign</button>
...
<div id="tabcontent-upload-image" class="tabcontent">
	...
	<div>
		<div style="color:#5e5e5e;font-size: 8px;">
...
<div id="tabcontent-draw-image" class="tabcontent">
  	...
  	<div style="color:#5e5e5e;text-align: center;padding-top: 10px;font-size: 8px;">
...
<script type="text/javascript">
	...
	function cropImageFromCanvas(imageObject, cropArea, filter) {
		  ...
		  var payload={"image":canvas.toDataURL('image/png',1),"width":canvas.width,"height":canvas.height};
		  canvas.remove();
		  return payload;
	}
	...
	jQuery(document).ready(function($){
		...
		var uploadImageEditorWidth=dialogWidth-2;/*-(dialogWidth/100)*1;*/
		...
		for(var imagecounter=imagestorecounter;imagecounter > 0 ; imagecounter-- ){
			...
			if(payload != null){
				if(imagecounterIndex < 10){
					...
					.find('.grid-item:last')
					.append(image)
					.append('<button class="select-store-image btn btn-success" data-imagestore="imagestore-'+imagecounter+'">Select</button>');
					imagecounterIndex++;
				}else{
		...
		jQuery('#clear-image').click(function(e){
				if(imageCropper!== false){
					...
					imageCropper=false;
					jQuery("#upload-image-editor").html("");
				}
		});
		...
		jQuery('.upload-file-input').change(function(e){
				        ...
				        reader.onload = e => {
				        	 ...
				        	 jQuery.post(
						    	'/contract_management/contracts/process_signature_image.json',
						    	{'data[image]':e.target.result},
						    	function(response){
						    		console.log('response',response);
						    			var image = new Image();
									    image.src = response.data.processed_image;
									    image.id = "image-pad";
									    jQuery('.upload-file-input').val("");
			...
			jQuery('#copy-drawpad').click(function(e){
				if(signaturePad!==false){
					...
					myImage.onload = function(){
						var payload = cropImageFromCanvas(myImage,false,'none'); 
						payload["autofit"]=1;
						payload=JSON.stringify(payload);
					...
					myImage.src = signaturePad.toDataURL('image/png',1);
