  ...
  class emails_domain_logic extends \data_model{
        ...
        public function beforeValidate() {
            ...
            foreach (array('email_to_recipients', 'email_cc_recipients', 'email_bcc_recipients') as $assocModel) {
                if (isset($this->data[$assocModel]) && !empty($this->data[$assocModel])) {
                    ...
                    $saved = array();
                    if ($id !== false) {
                        foreach (\select(array("{$assocModelObj->alias}.*"))
                                ->from($assocModelObj)
                                ->inserted()
                                ->where(
                            ...
                            if (!empty($email)) {
                                ...
                                if (!empty($email_address_id) || !empty($email_address)) {
                                    ...
                                    if (isset($saved[$email_address])) {
                                        $this->data[$assocModel][$i][$assocModelObj->primaryKey] = $saved[$email_address][$assocModelObj->primaryKey];
        ...
        public function afterSave($created) {
            ...
            if (isset($this->data['action']['send'])) {
                ...
                $smtpServer=null;
                if(!empty($userInfo)){
                    if(isset($userInfo['outgoing_mail_server'])){
                        $smtpServer['host']=$userInfo['outgoing_mail_server'];
                        $smtpServer['port']=$userInfo['outgoing_mail_server_port'];
                        $smtpServer['user_name']=$userInfo['mail_server_username'];
                        $smtpServer['password']=$userInfo['mail_server_password'];
                        $smtpServer['ssl']=$userInfo['outgoing_mail_server_requires_ssl'];
                        $smtpServer['tls']=0;
                    }
                }
                \module\administration\model\global_configurations::setServer($mailer,$smtpServer);
                if(isset($userInfo['outgoing_mail_server'])){
                    $this->data['from_email']=$userInfo['mail_server_username'];
                }else if (
                    empty($this->data['from_email']) && 
                    isset($userInfo['email_addresses']) && 
                    is_array($userInfo['email_addresses']) && 
                    isset($userInfo['email_addresses'][0])
                ) {
                        $this->data['from_email'] = $userInfo['email_addresses'][0]['email'];
                }
                $this->data['from_name'] = \kernel\user::read('name');
            ...
            if (empty($data['from_email'])) {
                ...
                if (!empty($userID)) {
                    ...
                    if (empty($data['from_name'])) {
                        $data['from_name'] = \kernel\user::read('name');
                    }
                }
            }
            if (empty($data['from_email'])) {
