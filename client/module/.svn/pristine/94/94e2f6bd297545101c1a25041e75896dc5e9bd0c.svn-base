...
<?php echo $scriptForLayout; ?>
    ...
    <body  class="ui-widget">
        <?php
        echo \tushar\html::js($this->request, '/module/documentation/js/menubar');
        ?>
    <br><br>
                ...
                <td style="width:80%;"  valign="top" class="no-mp">
                    <div id="content-panel" style="clear: both">
                        <div class="ui-widget-content" style="padding:10px;">
                            <?php
                            $prev = $next = false;
                            if (isset($nodeList) && !empty($nodeList) && is_array($nodeList)) {
                                $activeNodeID = $this->request->param('id');
                                $current = array_search($activeNodeID, $nodeList);
                                if (isset($nodeList[$current - 1])) {
                                    $prev = $nodeList[$current - 1];
                                }
                                if (isset($nodeList[$current + 1])) {
                                    $next = $nodeList[$current + 1];
                                }
                            }
                            ?>
                            <div><a href="<?php echo ($prev != false ? $this->request->base . $this->request->module . "/" . $this->request->controller . '/__view' . '?id=' . $prev : "#"); ?>"> Previous </a> | <a  href="<?php echo ($next != false ? $this->request->base . $this->request->module . "/" . $this->request->controller . '/__view' . '?id=' . $next : "#"); ?>">Next</a></div>
